...
export default function EditQuotePage() {
  ...
  const load = useCallback(async () => {
    ...
    if (res.ok) {
      ...
      setTitle(data.title ?? '')
      setExpiresAt(data.expiresAt ? String(data.expiresAt).slice(0, 10) : '')
      setNotes(data.notes ?? '')
  ...
  async function handleSave() {
    ...
    const res = await fetch(`/api/quotes/${id}`, {
      ...
      body: JSON.stringify({ title, expiresAt: expiresAt || null, notes: notes || null, lines }),
    })
    setSaving(false)
    if (res.ok) router.push(`/app/quotes/${id}`)
  ...
  if (!editable) {
    return (
      <div className="p-6 max-w-3xl mx-auto">
        <Link href={`/app/quotes/${id}`} className="text-blue-600 text-sm flex items-center gap-1 mb-4"><ArrowLeft className="w-4 h-4" />Retour</Link>
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 text-sm text-amber-900">
          Seuls les devis en <strong>brouillon</strong> peuvent être modifiés. Ce devis a déjà été envoyé.
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="p-4 sm:p-6 max-w-3xl mx-auto space-y-5">
      <Link href={`/app/quotes/${id}`} className="text-blue-600 text-sm flex items-center gap-1"><ArrowLeft className="w-4 h-4" />Retour au devis</Link>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-4 sm:p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="sm:col-span-2">
          <label className="block text-xs font-medium text-gray-600 mb-1">Titre du devis *</label>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-4 sm:p-6">
        ...
        <div className="mb-4 flex items-start gap-2 rounded-lg bg-amber-50 border border-amber-200 px-3 py-2 text-xs text-amber-900">
          ...
          <p>Vérifiez le taux de TVA applicable (20 % / 10 % / 5,5 % selon la nature des travaux et le logement) avant envoi.</p>
        </div>
      ...
      <div className="flex gap-3">
        <Link href={`/app/quotes/${id}`} className="px-4 py-2 rounded-lg border border-gray-300 text-sm">Annuler</Link>
